...
from monolayout import models as monolayoutmodels


def get_args():
    parser = argparse.ArgumentParser(description="Testing arguments for MonoLayout")
    parser.add_argument(
        "--image_path", type=str, help="path to folder of images", required=True
    )
    parser.add_argument(
        "--model_path", type=str, help="path to MonoLayout model", required=True
    )
    parser.add_argument(
        "--ext", type=str, default="png", help="extension of images in the folder"
    )
    parser.add_argument(
        "--out_dir", type=str, default="output directory to save topviews"
    )
...
def test(args):
    ...
    models["encoder"] = monolayoutmodels.MonoLayoutEncoder(feed_height, feed_width)
    filtered_dict_enc = {
        k: v for k, v in encoder_dict.items() if k in models["encoder"].state_dict()
    }
    ...
    if args.type == "both":
        ...
        models["static_decoder"] = monolayoutmodels.MonoLayoutDecoder()
        models["static_decoder"].load_state_dict(
            torch.load(static_decoder_path, map_location=device)
        )
        models["dynamic_decoder"] = monolayoutmodels.MonoLayoutDecoder()
        ...
        models["decoder"] = monolayoutmodels.MonoLayoutDecoder()
        ...
        for idx, image_path in enumerate(paths):
            ...
            if args.type == "both":
                ...
                save_topview(
                    idx,
                    static_tv,
                    os.path.join(args.out_dir, "static", "{}.png".format(output_name)),
                )
                save_topview(
                    idx,
                    dynamic_tv,
                    os.path.join(args.out_dir, "dynamic", "{}.png".format(output_name)),
                )
            else:
                ...
                save_topview(
                    idx,
                    tv,
                    os.path.join(args.out_dir, args.type, "{}.png".format(output_name)),
                )
