...
def get_args():
    ...
    parser.add_argument("--seg_dir", type=str, default="./kitti_raw",
                         help="Root directory for segmentation")
...
def get_road_velo_pts(velo_pts, frame_seg_img, Tr_cam2_velo, K, req_labels):
	...
	for req_label in req_labels:
		road_flag = np.logical_or(road_flag, frame_seg_img[image_pts[1, :], image_pts[0, :]] == req_label) 
...
#        return TopView
    ...
    #TopView = np.zeros((int(width/res), int(height/res)))
    for label in labels:
        if label == "":
            continue

        elems = label.split()
        if elems[0] in ['Car', 'Van', 'Bus']:
            center_x = int(float(elems[11])/res + width/(2*res))
            center_y = int(float(elems[12])/res)
            center_z = int(float(elems[13])/res)
            orient   = -1 * float(elems[14])

            obj_h = int(float(elems[8])/res)
            obj_w = int(float(elems[9])/res)
            obj_l = int(float(elems[10])/res)


            #if (center_x < 20) and (center_x > width/res-20) and (center_z < 20) and (center_z > height/res - 20):
             #   continue

            rectangle = get_rect(center_x, int(length/res)-center_z, obj_l, obj_w, orient)
            draw = ImageDraw.Draw(img)
            draw.polygon([tuple(p) for p in rectangle], fill=255)

    img = img.convert('L')
    file_path = os.path.join(out_dir, os.path.basename(label_path)[:-3]+"png")
    img.save(file_path)
...
def get_static_bev(args):
	...
	for frame_no in range(num_frames):
		...
		Tr_base_current_imu = oxts[frame_no].T_w_imu # to base imu frame from current imu frame
		Tr_base_cam2 = np.matmul(Tr_base_current_imu, np.matmul(np.linalg.inv(Tr_velo_imu), np.linalg.inv(Tr_cam2_velo))) # to base imu frame from cam2 frame
		...
		frame_seg_path = os.path.join(args.seg_dir, args.date, args.date + '_drive_' + args.sequence + '_sync', str(frame_no).zfill(10) + '.png')
		...
		dense_class_base_pts_hom = np.concatenate((dense_class_base_pts_hom, seg_class_base_pts_hom), axis = 1)
		print("reading velodyne points from frame " + str(frame_no).zfill(6))

	for frame_no in range(num_frames):

		Tr_current_base_imu = np.linalg.inv(oxts[frame_no].T_w_imu) # to current imu from base imu frame
		Tr_cam2_base = np.matmul(Tr_cam2_velo, np.matmul(Tr_velo_imu, Tr_current_base_imu)) # to cam2 frame from base imu frame 
		dense_class_cam_pts_hom = np.matmul(Tr_cam2_base, dense_class_base_pts_hom)
