...
def generate_weak_static(dataset_dir = "", log_id="", output_dir = ""):
	...
	for idx in range(len(lidar_timestamps)):
		...
		lidar_loc = os.path.join(dataset_dir, log_id, 'lidar', 'PC_' + str(lidar_timestamp) + '.ply')
		try:
			img_loc = argoverse_data.get_image_list_sync(camera = camera)[idx];
		except:
			print("Not finding image files")
		cam_timestamp = int(img_loc.split('.')[0].split('_')[-1])
		
		segment_loc = os.path.join(dataset_dir, log_id, 'segmentation', 'stereo_front_left_' + str(cam_timestamp) + '.png')
		segment_img = cv2.imread(segment_loc, 0)
		
		pc = argoverse_data.get_lidar(idx)
		uv = calib.project_ego_to_image(pc).T
		idx_ = np.where(np.logical_and.reduce((uv[0, :] >= 0.0, uv[0, :] < np.shape(segment_img)[1] - 1.0, uv[1, :] >= 0.0, uv[1, :] < np.shape(segment_img)[0] - 1.0, uv[2, :] > 0)))
		idx_ = idx_[0]
		uv1 = uv[:, idx_]
		uv1 = uv1.T
		...
		occ_map_loc = os.path.join(sparse_road_bev_loc, 'stereo_front_left_' + str(cam_timestamp) + '.png')
		...
		sparse_city_road_pts = city_to_egovehicle_se3.transform_point_cloud(lidar_road_pts)
		try:
			dense_city_road_pts = np.concatenate((dense_city_road_pts, sparse_city_road_pts), axis = 0)
		except:
			dense_city_road_pts = sparse_city_road_pts
	
	dense_road_bev_loc = os.path.join(output_dir, log_id, 'road_weak')
	try:
	...
	for idx in range(len(lidar_timestamps)):
		...
		translation = np.array(pose_data["translation"])
		city_to_egovehicle_se3 = SE3(rotation=quat2rotmat(rotation), translation=translation)
		to_current = city_to_egovehicle_se3.inverse()
		current_ego_frame_road_pts = city_to_egovehicle_se3.inverse_transform_point_cloud(dense_city_road_pts)
		current_cam_frame_road_pts = calib.project_ego_to_cam(current_ego_frame_road_pts)
		...
		occupancy_map[y_img, x_img] = 255
		occ_map_loc = os.path.join(dense_road_bev_loc, 'stereo_front_left_' + str(cam_timestamp) + '.png')
		cv2.imwrite(occ_map_loc, occupancy_map)
