...
def get_lane_bev(req_lane_ids, avm, city_name, city_to_egovehicle_se3, egovehicle_SE3_cam, top_view, res, lane_colour):

	for req_lane_id in req_lane_ids:

		polygon = avm.get_lane_segment_polygon(req_lane_id, city_name)

		local_lane_polygon = city_to_egovehicle_se3.inverse().transform_point_cloud(polygon)
		local_lane_polygon = egovehicle_SE3_cam.inverse().transform_point_cloud(local_lane_polygon)
...
def generate_vehicle_bev(dataset_dir = "", log_id="", output_dir = ""):
	...
	ind = 0
	for i in range(9):
		if calib_data['camera_data_'][i]['key'] == 'image_raw_stereo_front_left':
			ind = i
			break
	rotation = np.array(calib_data['camera_data_'][ind]['value']['vehicle_SE3_camera_']['rotation']['coefficients'])
	translation = np.array(calib_data['camera_data_'][ind]['value']['vehicle_SE3_camera_']['translation'])
	egovehicle_SE3_cam = SE3(rotation=quat2rotmat(rotation), translation=translation)
	city_path = f"{dataset_dir}/{log_id}/city_info.json"
	city_data = read_json_file(city_path)
	...
	pfa = PerFrameLabelAccumulator(dataset_dir, dataset_dir, "argoverse_bev_viz", save = False, bboxes_3d = True)
	pfa.accumulate_per_log_data(log_id)
	log_egopose_dict = pfa.log_egopose_dict
	...
	for i, ply_name in enumerate(ply_list):
		...
		cam_timestamp = sdb.get_closest_cam_channel_timestamp(lidar_timestamp, "stereo_front_left", str(log_id))
		image_path = os.path.join(output_dir, str(log_id), "car_bev_gt", "stereo_front_left_" + str(cam_timestamp) + ".jpg")
		...
		if not all_occluded:
			for i, frame_rec in enumerate(objects):
				...
				idx_ = np.all(np.logical_and(np.logical_and(np.logical_and(uv[0, :] >= 0.0, uv[0, :] < size[1] - 1.0),
                                                      np.logical_and(uv[1, :] >= 0.0, uv[1, :] < size[0] - 1.0)),
                                                      uv[2, :] > 0))
				if not idx_:
					continue
				bbox_cam_fr = egovehicle_SE3_cam.inverse().transform_point_cloud(bbox_ego_frame)
				color = frame_rec.color
				X = bbox_cam_fr[:, 0]
				Z = bbox_cam_fr[:, 2]	

				if (frame_rec.occlusion_val != IS_OCCLUDED_FLAG and frame_rec.obj_class_str == "VEHICLE"):
					y_img = (-Z / res).astype(np.int32)
					x_img = (X / res).astype(np.int32)
					x_img -= int(np.floor(-20 / res))
					y_img += int(np.floor(40 / res))
...
def generate_road_bev(dataset_dir = "", log_id="", output_dir = ""):
        ...
        images_list = os.listdir(os.path.join(dataset_dir, log_id, 'stereo_front_left'))
        ...
        lidar_timestamps = list(ply_locs[:, 1])
        ...
        for idx in range(len(lidar_timestamps)):
                ...
                pose_fpath = os.path.join(dataset_dir, log_id, "poses", "city_SE3_egovehicle_" + str(lidar_timestamp) + ".json")
                ...
                ego_car_nearby_lane_ids = avm.get_lane_ids_in_xy_bbox(xcenter, ycenter, city_name, 50.0)
                occupancy_map = get_lane_bev(ego_car_nearby_lane_ids, avm, city_name, city_to_egovehicle_se3, egovehicle_SE3_cam, occupancy_map, res, 255)
                output_loc = os.path.join(output_dir, log_id, 'road_gt', 'stereo_front_left_' + str(cam_timestamp) + '.png')
